...
/**
 * This is an implementation of the Zhang and Shasha algorithm as described in [FIXME]
 *
 * SWAN 2007-11-01: I'm pretty sure this code comes from: http://www.cs.queensu.ca/TechReports/Reports/1995-372.pdf and
 * "http://www.inf.unibz.it/dis/teaching/ATA/ata7-handout-1x1.pdf" INSERT-LICENCE-INFO
 */
public class ComparisonZhangShasha {
		...
		for (Integer aKeyroot : aTreeKeyRoots) { // aKeyroot loop
			...
			for (Integer bKeyroot : bTreeKeyRoots) { // bKeyroot loop
				...
				for (int i = aLeftLeaf.get(aKeyroot); i <= aKeyroot; i++) {
					setFD(i, bLeftLeaf.get(bKeyroot) - 1, getFD(i - 1, bLeftLeaf.get(bKeyroot) - 1, fD)
							+ ops.getOp(OpsZhangShasha.DELETE).getCost(i, 0, aTree, bTree), fD);
				...
				for (int j = bLeftLeaf.get(bKeyroot); j <= bKeyroot; j++) {
					...
					setFD(aLeftLeaf.get(aKeyroot) - 1, j, getFD(aLeftLeaf.get(aKeyroot) - 1, j - 1, fD)
							+ ops.getOp(OpsZhangShasha.INSERT).getCost(0, j, aTree, bTree), fD);
				...
				for (int i = aLeftLeaf.get(aKeyroot); i <= aKeyroot; i++) {
					...
					for (int j = bLeftLeaf.get(bKeyroot); j <= bKeyroot; j++) {
						...
						java.lang.Math.min(// Option 1: Delete node from aTree
								getFD(i - 1, j, fD) + ops.getOp(OpsZhangShasha.DELETE).getCost(i, 0, aTree, bTree),

								// Option 2: Insert node into bTree
								getFD(i, j - 1, fD) + ops.getOp(OpsZhangShasha.INSERT).getCost(0, j, aTree, bTree));
						...
						if ((aLeftLeaf.get(i) == aLeftLeaf.get(aKeyroot))
							...
							distance[i][j] = java.lang.Math.min(min, getFD(i - 1, j - 1, fD)
									+ ops.getOp(OpsZhangShasha.RENAME).getCost(i, j, aTree, bTree));
							...
							setFD(i, j,
									java.lang.Math.min(min,
											getFD(aLeftLeaf.get(i) - 1, bLeftLeaf.get(j) - 1, fD) + distance[i][j]),
									fD);
	...
	/**
	 * The initiating call should be to the root node of the tree. It fills in an nxn (hash) table of the leftmost leaf
	 * for a given node. It also compiles an array of key roots. The integer values IDs must come from the post-ordering
	 * of the nodes in the tree.
	 */
	private void findHelperTables(TreeDefinition someTree, Hashtable<Integer, Integer> leftmostLeaves,
	...
	private void findHelperTablesRecurse(TreeDefinition someTree, Hashtable<Integer, Integer> leftmostLeaves,
			ArrayList<Integer> keyroots, int aNodeID) {
	...
	private void seeFD(int a, int b, Hashtable<Integer, Hashtable<Integer, Double>> forestDistance) {

		System.out.println("[" + a + "],[" + b + "]: " + getFD(a, b, forestDistance));
	}

	/** Returns a String for trace writes */
	private void seeFD(Hashtable<Integer, Hashtable<Integer, Double>> forestDistance) {
