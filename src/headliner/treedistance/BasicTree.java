...
/**
 * This defines are tree. The tree contains labelled nodes. Labels must be unique. Sibling order is defined by the order
 * in which edges are added to a parent node.
 * 
 * Populating the tree is done at construction time, which must also specify what type of tree node traversal to number
 * all the nodes. Default is POSTORDER
 *
 * INSERT-LICENCE-INFO
 */
public class BasicTree extends TreeDefinition {
	 ...
	 */
	public BasicTree(Hashtable<String, ArrayList<String>> tree, String _root, int ordering) {
		setRoot(_root);
		treeStructure = tree;
